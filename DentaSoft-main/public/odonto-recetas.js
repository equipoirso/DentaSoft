...
(async function () {
  ...
  try {
    ...
    if (uid) {
      // Primero doc por id = uid
      let snap = await db.collection('odontologos').doc(uid).get();
      // Si no existe, buscar por campo uid
      if (!snap.exists) {
  ...
  $btnCrear.addEventListener('click', () => {
    ...
    // Datos del paciente (arriba del texto)
  ...
  function renderPaciente(p) {
    ...
    $box.innerHTML = `
      <div class="paciente-card" data-paciente
        ...
        <div class="pac-head">Paciente encontrado</div>
        <div class="pac-grid">
  ...
  function hideReceta(){ $wrapRec.classList.add('hidden'); $txt.value=''; }
